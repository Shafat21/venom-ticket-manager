...
module.exports = () => {
    ...
    if (config.credits) helpEmbed.setFooter({text:"Ticket System by VENOM#9208 | Development on progress"})
    ...
    client.on("messageCreate",msg => {
        ...
        if (!args[1].startsWith("close") && !args[1].startsWith("delete") && !args[1].startsWith("remove") && !args[1].startsWith("add") && !args[1].startsWith("msg") && !args[1].startsWith("remove")  && !args[1].startsWith("rename") && !args[1].startsWith("say")){

            msg.channel.send({embeds:[helpEmbed]})
            if (config.logs){console.log("[command] "+config.prefix+"help (user:"+msg.author.username+")")}
        }
    })
